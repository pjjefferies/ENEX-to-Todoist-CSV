...
def load_enex_backup(
    ...
    # include_images_in_notes: bool = False,
    ...
    for note_no, note in enumerate(data):
        ...
        for field in note:
            match field.tag:
                case "content":
                    ...
                    temp = parser.data.strip().replace("\xa0", " ")
                            ...
                            case "resource-attributes":
                                for sub_sub_field in sub_field:
                                    match sub_sub_field.tag:
                                        ...
                                        case _:
                                            logger.debug(
                                                msg=f"Ignoring image field: {sub_sub_field.tag=}"
                                            )
                                if image_data["file_name"] is None:
                                    ...
                                    # logger.debug(
                                    #     msg=f"image filename changed from None to {image_data['file_name']}"
                                    # )
                            case _:
                                logger.debug(
                                    msg=f"Ignoring image field: {sub_field.tag=}"
                                )
                    if all(
                        ...
                        logger.debug(
                            msg=f"Didn't find all image data. Have: {image_data.keys()}, hash: {image_data['hash']}"
                        )
                ...
                case _ if field.tag != "tag":
                    ...
                    # logger.debug(msg=f"{note_no}: {field.tag=}, {field.text=}")
                case _:  # if field.tag == "tag"
        ...
        if (
            ...
            logger.debug(
                msg=f"Skipping note {note_no} due to empty title and content. {this_note=}"  # , {[field.data for field in note]}"
            )
...
if __name__ == "__main__":
    ...
    for note_no, note_dict in enumerate(notes):
        logger.debug(msg=f"Note No.: {note_no}:")
        logger.debug(msg=f"{note_dict}")
        if note_no >= MAX_NOTES_TO_SHOW:
            break

    for resource_no, resource_dict in enumerate(resources):
        logger.debug(msg=f"Resource No.: {resource_no}:")
        logger.debug(msg=f"{resource_dict['hash']=}, {resource_dict['file_name']=}")
